...
export const calulateDisplayType = original => {
  ...
  else if (field.type === SWAGGER_FIELD_FORMATS.BOOLEAN)
    field.format = VALID_FORMATS.CHECKBOX
  else if (field.type === SWAGGER_FIELD_FORMATS.TIMESTAMP)
    field.format = VALID_FORMATS.DATETIME
  else if (field.type === SWAGGER_FIELD_FORMATS.JSONB)
    field.format = VALID_FORMATS.TEXTAREA
  else if (
    field.type === SWAGGER_FIELD_FORMATS.INTEGER ||
    field.type === SWAGGER_FIELD_FORMATS.BIGINT
  )
    field.format = VALID_FORMATS.NUMBER
  else if (
    field.type === SWAGGER_FIELD_FORMATS.STRING &&
    field.format === SWAGGER_FIELD_FORMATS.BIGINT
  )
    field.format = VALID_FORMATS.NUMBER
  else if (
    field.type === SWAGGER_FIELD_FORMATS.STRING &&
    field.format === SWAGGER_FIELD_FORMATS.TEXT
  )
    field.format = VALID_FORMATS.TEXTAREA
  else if (
    field.type === SWAGGER_FIELD_FORMATS.STRING &&
    field.format === SWAGGER_FIELD_FORMATS.CHARACTER_VARYING
  )
    field.format = VALID_FORMATS.TEXT
  else if (
    field.type === SWAGGER_FIELD_FORMATS.STRING &&
    field.format === SWAGGER_FIELD_FORMATS.DATE
  )
    field.format = VALID_FORMATS.DATE
  else if (
    field.type === SWAGGER_FIELD_FORMATS.STRING &&
    field.format === SWAGGER_FIELD_FORMATS.TIMESTAMP_NO_TIMEZONE
  )
    field.format = VALID_FORMATS.DATETIME
  return field
}
...
export const hasDataChanged = field => {
  if (!field.modifiedValue || typeof field.modifiedValue === 'undefined')
    return true
  switch (field.type) {
    ...
    case SWAGGER_FIELD_FORMATS.JSON:
      return (
        ...
        JSON.stringify(field.modifiedValue)
      )
    default:
...
export const isForeignKey = field => {
  if (field.description && field.description.indexOf('<fk') >= 0) {
    let fkString = field.description.substring(
      field.description.indexOf('<fk') + 4,
      field.description.indexOf('/>')
    )
    let fkParts = fkString.split(' ')
    return {
      table: fkParts[0].substring(
        fkParts[0].indexOf("'") + 1,
        fkParts[0].lastIndexOf("'")
      ),
      column: fkParts[1].substring(
        fkParts[1].indexOf("'") + 1,
        fkParts[1].lastIndexOf("'")
      ),
    }
  } else return false
}

export const getUniqueSelector = (primaryKeys, record) => {
...
export const verifySelectorReturnsUnique = async (
  context,
  appId,
  resourceKey,
  selector
  ...
  const url = `/api/postgrest/${appId}/${resourceKey}?q=${encodeURIComponent(
    selector
  )}`
  console.log('url', url)
  let { data: proxyResponse } = await context.$axios.get(url).catch(e => {
    throw e
  })
  return proxyResponse && proxyResponse.data && proxyResponse.data.length === 1
    ? true
    : false
}

// must be called from a page or component, passing in the context
export const createRecord = (context, appId, resourceKey, data) => {
...
// must be called from a page or component, passing in the context
export const deleteRecord = (context, appId, resourceKey, selector) => {
  const url = `/api/postgrest/${appId}/${resourceKey}?q=${encodeURIComponent(
    selector
  )}`
...
// must be called from a page or component, passing in the context
export const getRecord = (context, appId, resourceKey, selector) => {
  console.log('selector', selector)
  const url = `/api/postgrest/${appId}/${resourceKey}?q=${encodeURIComponent(
    selector
  )}`
...
// must be called from a page or component, passing in the context
export const updateRecord = (context, appId, resourceKey, selector, data) => {
  const url = `/api/postgrest/${appId}/${resourceKey}?q=${encodeURIComponent(
    selector
  )}`

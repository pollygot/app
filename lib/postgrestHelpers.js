...
export const verifySelectorReturnsUnique = async (context, appId, resourceKey, selector) => {
  ...
  console.log('url', url)
  let { data: proxyResponse } = await context.$axios.get(url).catch(e => { throw e })
...
export const createRecord = (context, appId, resourceKey, data) => {
  ...
  return context.$axios.post(url, data).catch(e => { throw e })
}

// must be called from a page or component, passing in the context
export const deleteRecord = (context, appId, resourceKey, selector) => {
  const url = `/api/postgrest/${appId}/${resourceKey}?q=${encodeURIComponent(selector)}`
  return context.$axios.delete(url).catch(e => { throw e })
}
...
export const getRecord = (context, appId, resourceKey, selector) => {
  ...
  return context.$axios.get(url, headers).catch(e => { throw e })
}

// must be called from a page or component, passing in the context
export const updateRecord = (context, appId, resourceKey, selector, data) => {
  const url = `/api/postgrest/${appId}/${resourceKey}?q=${encodeURIComponent(selector)}`
  return context.$axios.patch(url, data).catch(e => { throw e })
}
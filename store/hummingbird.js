...
export const getters = {
  customView: state => viewId =>
    state.customViews.find(view => view.id == viewId),
  ...
  columnsForResource: state => resourceKey => {
    ...
    let columnArray = Object.entries(tableDefinition.properties).map(([k, v]) =>
      Object.assign({ ...v, key: k, resource: resourceKey }, v)
    )
    return columnArray
  ...
  primaryKeysForResource: state => resourceKey => {
    ...
    let columns = Object.entries(properties).map(([k, v]) =>
      Object.assign({ ...v, key: k }, v)
    ) // convert to array
    return columns
